...
export default function Products() {
    ...
    const [name, setName] = useState('');
    const [description, setDescription] = useState('');
    const [richDescription, setRichDescription] = useState(''); // Added to prevent DB overwrite
    const [price, setPrice] = useState('');
    const [category, setCategory] = useState('');
    const [countInStock, setCountInStock] = useState('');
    const [brand, setBrand] = useState('');
    ...
    const handleEditClick = (product) => {
        ...
        setName(product.name);
        setDescription(product.description);
        setRichDescription(product.richDescription || ''); // Load existing rich text
        setPrice(product.price);
        setCategory(product.category?.id || product.category || '');
        setCountInStock(product.countInStock);
        setBrand(product.brand || '');
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('name', name);
        formData.append('description', description);
        formData.append('richDescription', richDescription); // Append safely
        formData.append('price', price);
        formData.append('category', category);
        formData.append('countInStock', countInStock);
        formData.append('brand', brand);
            ...
            <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div className="glass-panel" style={{ padding: '30px', flex: '1', minWidth: '350px' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        {/* Rich Description text area to keep the DB schema happy */}
                        ...
                        <div style={{ display: 'flex', gap: '15px' }}>
                            <input className="glass-input" type="number" placeholder="Price (₹)" value={price} onChange={(e) => setPrice(e.target.value)} required min="0" style={{ flex: 1 }} />
                            {/* Max 255 enforced to match Joi schema limitations */}
