/**
 * @fileoverview Admin User Management Component.
 * Handles the complete CRUD lifecycle for user accounts. Allows administrators to
 * view the customer directory, edit details, assign/revoke Admin privileges, and delete accounts.
 */
...
/**
 * Users Component
 * @returns {JSX.Element} The rendered user directory and management interface.
 */
export default function Users() {
    ...
    /**
     * Fetches a paginated list of users from the backend directory.
     */
    ...
    /**
     * Triggers a custom confirmation toast before permanently deleting a user account.
     * Displays a distinct warning if the target is an Administrator.
     * @param {string} userId - The database ID of the user to delete.
     * @param {boolean} userIsAdmin - Flag indicating if the user has admin privileges.
     */
    ...
    /**
     * Opens the modal in "Edit Mode" and populates the form with existing user data.
    ...
    const openEditModal = (user) => {
        ...
        setEmail(user.email);
        setPhone(user.phone);
        setPassword(''); // Always leave password blank unless explicitly changing it
        setIsAdmin(user.isAdmin);
        setIsModalOpen(true);
    };

    /**
     * Opens the modal in "Creation Mode" with a clean slate.
     */
    const openAddModal = () => {
        setEditingUser(null);
        setName('');
    ...
    /**
     * Closes the modal and resets the editing state to prevent data bleed.
     */
    const closeModal = () => {
    ...
    /**
     * Submits the form data to either create a new user or update an existing one.
     */
    const handleUserSubmit = async (e) => {
        ...
        try {
            if (editingUser) {
                // Construct the payload, safely preserving existing address details
                // so they are not accidentally overwritten or wiped by an admin update.
                const payload = { 
                ...
                // Only append the password field if the admin actually typed a new one
            ...
            toast.error(err.response?.data?.message || err.response?.data || `Failed to process user profile.`);
        } finally {
            setUpdating(false);
        }
    };

    // Client-side filtering of the currently loaded page of users
